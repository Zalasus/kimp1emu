...
#define Z80_READ_BYTE(address, x)   (x) = readByte((address) & 0xffff, ((KIMP_CONTEXT*)context));

#define Z80_FETCH_BYTE(address, x)  Z80_READ_BYTE((address), (x))


#define Z80_READ_WORD(address, x)   (x) = readWord((address) & 0xffff, ((KIMP_CONTEXT*)context));

#define Z80_FETCH_WORD(address, x)	Z80_READ_WORD((address), (x))


#define Z80_WRITE_BYTE(address, x)  writeByte((address) & 0xffff, (x), ((KIMP_CONTEXT*)context));

#define Z80_WRITE_WORD(address, x)  writeWord((address) & 0xffff, (x), ((KIMP_CONTEXT*)context));

#define Z80_WRITE_WORD_INTERRUPT(address, x)   Z80_WRITE_WORD((address), (x))


#define Z80_INPUT_BYTE(port, x)     (x) = ioRead((port), ((KIMP_CONTEXT*)context));

#define Z80_OUTPUT_BYTE(port, x)    ioWrite((port), (x), ((KIMP_CONTEXT*)context));
